...
def calculate_distribution_plan(tables, urun):
    ...
    if urun["depolama_kosulu"] not in ["Soğuk(+4)", "Donuk(-18)"]:
        merge_operations.append((tables["Raf Sepet Bilgi Tablosu"], ["magaza_kodu"], ["raf_sayisi", grup_kodu_sutun]))
    for table, on_columns, columns in merge_operations:
        dagitim_verileri = dagitim_verileri.merge(table[on_columns + columns], on=on_columns, how="left")
    if urun["depolama_kosulu"] not in ["Soğuk(+4)", "Donuk(-18)"]:
    ...
    if urun["yeni_mi"] == "yeni":
        normalized_columns = {"urun_grubu_ciro": 0.24, "ust_mal_grubu_ciro": 0.10, "raf_sayisi": 0.01, "raf_arasi_sayisi": 0.26, "gs_ciro": 0.21, "ortalama_ciro": 0.25, "hangi_ilce_score": 0.00, "magaza_tipi_score": 0.01}
    else:
        normalized_columns = {"urun_grubu_ciro": 0.16, "ust_mal_grubu_ciro": 0.14, "raf_sayisi": 0.01, "raf_arasi_sayisi": 0.20, "gs_ciro": 0.12, "ortalama_ciro": 0.18, "hangi_ilce_score": 0.00, "magaza_tipi_score": 0.01}
    ...
    column_order = ["magaza_kodu", "magaza_adi", "urun_kodu", "urun_adi", "dagitilan_koli", "skor", "stok", "satis", "urun_grubu", "ust_mal_grubu", "urun_grubu_ciro", "ust_mal_grubu_ciro", "raf_sayisi", "raf_arasi_sayisi", "magaza_tipi", "hangi_ilce", "gs_ciro", "ortalama_ciro", "stok_orani", "satis_hizi", "hangi_ilce_score", "magaza_tipi_score", "kalan", "depolama_kosulu"]
...
def calculate_kasa_aktivitesi(tables, urun):
    ...
    dagitim_verileri["hangi_ilce_score"] = dagitim_verileri["hangi_ilce"].apply(map_hangi_ilce_score)
    st.info("🔍 Top 40 belirleniyor...")
    if urun.get("yeni_mi", "eski") == "yeni":
        weights = {"toplam_satis_gucu": 0.50, "ks_ciro": 0.25, "ust_mal_grubu_ciro": 0.05, "mal_grubu_ciro": 0.10, "hangi_ilce_score": 0.05}
    else:
        weights = {"toplam_satis_gucu": 0.50, "ks_ciro": 0.25, "hangi_ilce_score": 0.05}
    ...
    if "stok_4hafta_sonunda" in dagitim_verileri.columns:
        birikim = dagitim_verileri.apply(lambda x: (x["stok"] / x["stok_4hafta_sonunda"]) if x["stok_4hafta_sonunda"] > 0 else 0, axis=1)
    ...
    if koli_per_magaza >= 15:
        base_minimum = 5
        st.info(f"ℹ️ Ortalama {koli_per_magaza:.1f} → Min: 5")
    elif koli_per_magaza >= 10:
        base_minimum = 4
        st.info(f"ℹ️ Ortalama {koli_per_magaza:.1f} → Min: 4")
    elif koli_per_magaza >= 5:
        base_minimum = 3
        st.info(f"ℹ️ Ortalama {koli_per_magaza:.1f} → Min: 3")
    else:
        base_minimum = 2
        st.info(f"ℹ️ Ortalama {koli_per_magaza:.1f} → Min: 2")
    if urun.get("yeni_mi", "eski") == "eski":
        ...
        for min_val, count in summary.items():
            if min_val == 0:
                ...
                st.write(f"  - 0: {count} (Top40: {top40_c}, Stok: {stok_c})")
            else:
                st.write(f"  - {int(min_val)}: {count}")
        minimum_total = dagitim_verileri["minimum_koli"].sum()
    else:
        st.info("🆕 Yeni ürün - Top 40 min=0")
        dagitim_verileri["minimum_koli"] = base_minimum
        dagitim_verileri.loc[top_40_magazalar, "minimum_koli"] = 0
        st.write("📊 Minimum:")
        st.write(f"  - 0: 40 (Top 40)")
        st.write(f"  - {base_minimum}: 164 (Diğer)")
        minimum_total = 164 * base_minimum
    if total_koli < minimum_total:
        st.warning("⚠️ Yetersiz koli, minimum azaltılıyor...")
    ...
    column_order = ["magaza_kodu", "magaza_adi", "urun_kodu", "urun_adi", "dagitilan_koli", "minimum_koli", "top_40", "skor", "stok", "satis_4hafta", "ara_satis", "toplam_satis_gucu", "ks_ciro", "hangi_ilce", "hangi_ilce_score", "stok_orani"]
    if urun.get("yeni_mi", "eski") == "yeni":
...
if kategori == "Kasa Aktivitesi":
    ...
    with col2:
        stok_satis_file = st.file_uploader("2️⃣ Stok-Satış", type=["xlsx"], key="stok_satis", help="ZORUNLU")
else:
    st.subheader("📥 Dosya Yükleme")
    col1, col2 = st.columns(2)
    with col1:
        urun_bilgisi_dosyasi = st.file_uploader("1️⃣ Ürün Bilgisi", type=["xlsx"], key="urun_bilgi")
    with col2:
        stok_satis_file = st.file_uploader("2️⃣ Stok-Satış (Opsiyonel)", type=["xlsx"], key="stok_satis")
...
if kategori == "Grup Spot":
    ...
    if not stok_satis_file:
        st.warning("⚠️ Stok-Satış zorunlu")
        st.stop()
    tables = {
...
if missing_tables:
    st.error(f"❌ Eksik: {', '.join(missing_tables)}")
else:
    st.success("✅ Hazır")
...
if urun_bilgisi_dosyasi and not missing_tables:
    ...
    st.success(f"✅ {len(urun_bilgisi)} ürün")
